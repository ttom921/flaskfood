...
def _echo_socket(ws):
    ...
    # appid=id(app)
    # print(f"_echo_socket app={ appid }")
    with app.app_context():
        print(f"app.name={app.name}")
    # 加入列表
    appManager.users[ws.id] = ws
    app.logger.info(f"加入列表={ws.id}")
    app.logger.info(f"列表={appManager.users}")
    while ws.connected :
        time.sleep(1)
        app.logger.info(f"{ws.id}==>wsocket.route=============")
        try:
            recdata=ws.receive()
            #len(recdata)
            #app.logger.info('reclent={0}'.format(len(recdata)))
        ...
        if recdata is not None:     
            if len(recdata) == 0:
                continue  
            msg=json.loads(recdata)  
            app.logger.info(f"現有連接用戶：{len(appManager.users)}") 
            app.logger.info(f"服務器收到 data={msg}")
            if msg:
                now = datetime.datetime.now()
                sendmsg=f"{msg['message']} {now}"
                msg['message']=sendmsg
                sendata=json.dumps(msg)
                #print(f'服務器 傳送:{sendata}')
                for id in appManager.users:
                    try:
                        app.logger.info(f"送給{appManager.users[id].id} {sendata}")
                        appManager.users[id].send(sendata)
                    except WebSocketError:
                        print('用戶已斷開連接')
                    # for client in client_dict.values():
                    #     try:
    ...
    #close wesocket
    app.logger.info(f'關畢 socket={ws.id}')
    del appManager.users[ws.id] 
