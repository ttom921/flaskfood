...
def main():
    ...
    try:
        ...
        user = User(name="沐言優拓-小李老師", age=16, birthday=bir_date, salary=24000.00, note="www.example.com")
        logging.info("--------------------------通過session保存數據--------------------------------")
        session.add(user)  # 瞬時態進入到預備態（與session關聯）
        session.commit()  # 事務提交（預備態變為了持久態）
        logging.info("--------------------------第一次session事務提交--------------------------------")
        user.name = "爆可愛的小李老師"  # 修改持久態下的屬性內容
        logging.info("--------------------------第二次session事務提交--------------------------------")
        session.commit()  # 事務提交（預備態變為了持久態）
